...
const float uvGridSize = 50;

bool uvStripes(float pos) {
    return mod(pos, 2.0 / uvGridSize) <= (1.0 / uvGridSize);
}
...
void main() {
    ...
    vec3 sunlightColor = getSunColor(lightDirection.y);

    vec3 ambient = vec3(0.1, 0.1, 0.2);
    vec3 diffuse = sunlightColor * max(dot(v_normal, lightDirection), 0.0) * u_sunlightTransmission;
    vec3 albedo = vec3(mix(0.8, 1.0, uvStripes(v_uv.x) ^^ uvStripes(v_uv.y)));
    float shadow = calculateShadow(v_lightSpaceFragPos, lightDirection);

    vec3 color = (ambient + (1.0 - shadow) * diffuse) * albedo;
