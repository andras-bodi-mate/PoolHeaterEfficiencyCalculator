...
void main() {
    ...
    float illumination = dot(v_normal, lightDirection) * u_sunlightTransmission;
    vec3 color = vec3(1.0) * clamp(illumination, 0.1, 1.0); //illumination > 0.0 ? vec3(0.0, clamp(illumination, 0.1, 1.0), 1.0) : vec3(1.0, 0.0, 0.0);
